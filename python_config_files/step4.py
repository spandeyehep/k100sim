...
for i in tqdm(range(len(EV_step1))):
	...
	if(ncap_index >= 0 and counter < len(n)):
		if(counter+1 != EV_step3[step3_evnt_counter]):
			# do nothing
			# save zeros
			empty_array = array('d')
			...
			for j,TS in enumerate(TS_step3[step3_evnt_counter]):
				...
				if(step != 1):
					continue
				if((X1_step3[step3_evnt_counter][j] != X1_step1[i][ncap_index]) or (Y1_step3[step3_evnt_counter][j] != Y1_step1[i][ncap_index]) or (Z1_step3[step3_evnt_counter][j] != Z1_step1[i][ncap_index])):
					print("ERROR : Sanity check failed.")
					print("-------See below for details---------")
					print("step1 entry = ",i)
					print("step3 entry = ",step3_evnt_counter)
					print("step1 neutron capture location = ({0},{1},{2})".format(X1_step1[i][ncap_index], Y1_step1[i][ncap_index], Z1_step1[i][ncap_index]))
